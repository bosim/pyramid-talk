...
@view_config(route_name='article_list', renderer='templates/article_list.pt')
def article_list(request):
   ...
   for db_result in db_results:
       results.append({
           'id': db_result.id,
           'url': route_url('show_article', request, article=str(db_result.id)),
...
@view_config(route_name='show_article', renderer='templates/show_article.pt')
def show_article(context, request):
   article = {}
   db_result = DBSession.query(Article).filter(Article.id==context.article).first()

   if not db_result:
       raise HTTPNotFound("Article could not be found")
